...
docker run --rm -v $(pwd)/python/hello:/io -v $(pwd)/rust:/rust  ghcr.io/pyo3/maturin build --release  --compatibility=manylinux_2_17  --interpreter python3.8 --auditwheel check
...
/usr/local/python/current/bin/hello 
